...
const Services = (props: IProps) => {
    const { bg, customClassName } = props;
    const background = `services ${bg} ${customClassName ?? ''}`;
    const isAboutPage = window.location.pathname === '/about';
    const smallTitle = isAboutPage ? 'About Us' : 'What we do'
    return (
        <section id="services" className={background}>
            <div className="container text-center mb-2">
                <h1 className="small-title">{smallTitle}</h1>
                {isAboutPage ? (
                    <>   
                        <div className="">
